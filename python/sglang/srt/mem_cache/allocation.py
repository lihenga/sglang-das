...
from sglang.srt.utils.common import is_pin_memory_available, pin_host_metadata
...
def alloc_for_extend(
    ...
    pin_metadata = pin_host_metadata(batch.device)
    prefix_lens_cpu = torch.tensor(
        batch.prefix_lens, dtype=torch.int64, pin_memory=pin_metadata
    )
    extend_lens_cpu = torch.tensor(
        batch.extend_lens, dtype=torch.int64, pin_memory=pin_metadata
    )
    ...
    req_pool_indices_cpu = torch.tensor(
        req_pool_indices, dtype=torch.int64, pin_memory=pin_metadata
    )
