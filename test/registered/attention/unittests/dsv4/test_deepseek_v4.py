        ...
        for num_qo_tokens, builds in (
            ...
            with self.subTest(num_qo_tokens=num_qo_tokens):
                ...
                with (
                    ...
                    mock.patch(
                        "sglang.srt.layers.attention.deepseek_v4_backend.is_dsa_enable_prefill_cp",
                        return_value=False,
                    ),
                    mock.patch(
                ...
                # Dense declares the boundary too; it reads only metadata that
                # init_forward_metadata already snapshotted.
                self.assertTrue(metadata.prefill_shared_reads_snapshotted)

    def test_legacy_cp_defers_sparse_prefill_snapshot_until_after_reindex(self):
        from sglang.srt.environ import envs
        from sglang.srt.layers.attention.base_attn_backend import SharedReadEnds
        from sglang.srt.layers.attention.deepseek_v4_backend import (
            DeepseekV4AttnBackend,
        ...
        with (
            ...
            backend.prepare_prefill_shared_read_snapshot(
                batch, num_qo_tokens=len(batch.input_ids)
            )

        self.assertIs(
            backend.shared_read_ends(ForwardMode.EXTEND), SharedReadEnds.UNKNOWN
        )
        backend._build_sparse_prefill_chunk_cache.assert_not_called()
        self.assertIsNone(backend.forward_metadata.sparse_prefill_cache)
